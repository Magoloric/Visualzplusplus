(function ($) {
  ...
  $.fn.visualize = function (preset) {
    ...
    var cw = preset.cw,
      ...
      ih = this.height(),
      imgID = this.attr('id'),
      imgCl = this.attr('class'),
      repID = imgID ? ' id="' + imgID + '"' : '',
      repCl = imgCl ? ' class="' + imgCl + '"' : ''
    if (cw === '100%') cw = iw;
    if (ch === '100%') ch = ih;
    var hcc = Math.ceil(iw / cw),
      vcc = Math.ceil(ih / ch),
      hrm = iw % cw,
      vrm = ih % ch,
      $replacement = $('<div' + repID + repCl + ' style="height:' + ih + 'px; width:' + iw + 'px;" class="grid-wrap">' + this[0].outerHTML + '</div>')
  ...
  var presets = {
    ...
    //May look weird on some pics, use with caution
    smallPGrams45plus: {
      cw: 50,
      ch: 35,
